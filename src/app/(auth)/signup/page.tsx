...
export default function SignupPage() {
    ...
    async function handleSignup(e: React.FormEvent) {
        ...
        // If auto-confirmed (local dev), create profile and redirect
        if (authData.user && authData.session) {
            const { error: profileError } = await supabase
                .from('profiles')
                .insert({ id: authData.user.id, name: name.trim() })
    ...
    if (success) {
        return (
            <>
                ...
                <Link href="/login" className="mt-6 text-blue-600 font-medium hover:underline">
                    Back to login
                </Link>
            </>
        )
    ...
    return (
        <>
            ...
            <div className="text-center mb-8">
                ...
                <p className="text-gray-500 mt-1">Get started with CartBuddy</p>
            </div>

            {/* Signup Card */}
            <div className="w-full max-w-sm">
                <div className="bg-white rounded-2xl shadow-xl shadow-gray-200/50 p-6 space-y-5 border border-gray-100">
                    {/* Google Sign Up */}
                    <button
                        onClick={handleGoogleSignup}
                        className="w-full py-3 px-4 bg-white border-2 border-gray-200 rounded-xl font-medium text-gray-700 hover:bg-gray-50 hover:border-gray-300 transition-all duration-200 flex items-center justify-center gap-3"
                    >
                        ...
                        <div>
                            ...
                            <div className="relative">
                                <input
                                    ...
                                    className="w-full px-4 py-3 pr-12 rounded-xl border border-gray-200 focus:border-blue-500 focus:ring-2 focus:ring-blue-500/20 outline-none transition-all text-gray-900 placeholder:text-gray-400"
                                />
                                <button
                                    type="button"
                ...
                <p className="text-center text-sm text-gray-500 mt-6">
                    ...
                    <Link
                        href={`/login${redirectTo !== '/households' ? `?redirect=${encodeURIComponent(redirectTo)}` : ''}`}
